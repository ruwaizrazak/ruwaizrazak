...
- **Verify a third-party library's behaviour in its source before a test asserts it.** A plan claimed tippy's `inlinePositioning` puts a wrapped link's tooltip above the line under the pointer; `getInlineBoundingClientRect` (`tippy.js/dist/tippy.esm.js`) anchors `placement: 'top'` to the **first** line and follows the pointer only for exactly two disjoint rects. Codex implemented the plan faithfully and the plan-written test failed. Read the function, or run a throwaway probe, in Phase 0, then write the test for the behaviour the library actually has.
- **Plan-supplied exact text must obey this file's rules.** A plan that prescribes a four-line `LEARN:` comment conflicts with the 1-2 line comment rule, and Codex rightly stops on it. The executor may not bend a repo rule to match a plan, so the plan has to be written within the rules in the first place. This covers test files too: a plan-written integrity test that opened with a three-line JSDoc stopped a run the same way. Before handing a plan over, check every comment in every exact block, whether source or test, against the comment rule.
- **Never build a test fixture on a network fetch made at build time.** External link tooltips come from `ogFetcher.ts`, which swallows fetch failures and returns empty metadata, so the `data-link-tooltip` attribute silently disappears whenever the site is unreachable. A test that selects an external link's tooltip therefore passes or fails with the network. Pick an internal link, whose tooltip resolves from the content collection.
- **A new key into a derived asset path needs its asset named in the plan.** `NotePostHero` builds its eyebrow glyph as `/icons/${collectionKey}.svg`. A plan that passed `collection="live"` would have pointed a CSS mask at a `live.svg` that does not exist, rendering an empty icon with no error anywhere. Codex caught it in the read pass. Whenever a plan introduces a new value into a template-built path (icons, images, routes), check that the file exists, or decide the fallback and assert the URL in a test.

- **`define:vars` wraps its script in a function, so nothing declared inside is global.** `<script define:vars={{ GA_ID }}>` compiles to `(function(){ const GA_ID = …; … })();`. The gtag snippet's `function gtag(){…}` was therefore local, `window.gtag` was `undefined` on production, and `analytics.ts`'s guarded `track()` silently dropped every custom GA event (`contact_click`, `work_view`, `scroll_depth`) while page views, pushed straight onto `dataLayer`, kept working. Anything another module reads off `window` must be assigned explicitly (`window.gtag = function () {…}`), and a guard like `typeof window.x === 'function'` needs a test that proves the branch is ever taken.
...
- **Never use `<svelte:element>` inside a link or button in a hydrated island.** Hydrating a dynamic element detaches it and re-inserts it. A press that starts before hydration and ends after it began on a node that briefly left the document, so no `click` fires. In Chromium, WebKit and mobile-chrome alike, a related card simply didn't navigate. For a variable heading level, branch on static tags: `{#if level === 2}<h2 …>{:else}<h3 …>{/if}`, or a snippet that does, like `titleHeading` in `ContentCard.svelte`. The markup is identical and it hydrates in place. `<svelte:element>` is still fine for elements nobody clicks. `tests/e2e/related-notes.spec.ts` guards this: it holds the island script so the press straddles hydration.
