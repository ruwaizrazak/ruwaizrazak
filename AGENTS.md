...
- **Plan-supplied exact text must obey this file's rules.** A plan that prescribes a four-line `LEARN:` comment conflicts with the 1-2 line comment rule, and Codex rightly stops on it. The executor may not bend a repo rule to match a plan, so the plan has to be written within the rules in the first place.
