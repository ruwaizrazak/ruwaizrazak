<script lang="ts">
  ...
  export interface ResolvedEntry {
    ...
    website: string;
    bullets: string[];
    /** Resolved by StickyExperience.astro (astro:assets is Astro-only). */
    logo: OptimizedImg | null;
    /** Pre-rendered tooltip markup for the company website link. */
  ...
  //   Be careful going above ~30 with many cards — the last card's sticky position
  //   (BASE_TOP_VH + entries.length * OFFSET_PX) must stay within the viewport.
  //
  // OFFSET_PX — pixel gap between each stacked card, i.e. how much of the previous
  //   card's header peeks out. ~60px ≈ the company header bar height.
  const BASE_TOP_VH = 25;
  const OFFSET_PX = 60;
</script>

<div class="sticky-experience">
  {#each entries as entry, i (entry.company + entry.period)}
    <div
    ...
    >
      <!-- Header bar — this strip stays visible when the next card stacks on top.
           Its height should roughly match OFFSET_PX so headers line up. -->
      <div class="card-header flex items-center justify-between gap-4 px-6 pt-5 pb-3 md:px-8 md:pt-6 md:pb-4">
        <div class="flex flex-row items-center gap-4">
          <OptimizedImage image={entry.logo} alt={entry.company} class="w-16 aspect-square" />
          <div>
            <h3 class="text-xl md:text-2xl font-sans font-bold uppercase tracking-wide text-syoro dark:text-white">
              {entry.company}
            </h3>
            <LinkView href={entry.website} tooltipHTML={entry.websiteTooltip} isExternal>
              Visit website
            </LinkView>
          </div>
        </div>
