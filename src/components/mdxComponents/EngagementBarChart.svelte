<script lang="ts">
  ...
  // Heights are computed at build time and inlined as style="height: X%", so zero JS ships.
  // Being script-free is also why this goes straight into the MDX components map
  // rather than needing an .astro client:* wrapper.
  interface Metric {
    label: string;
    before: number;
    after: number;
  ...
  <div class="grid grid-cols-1 sm:grid-cols-3 gap-10 sm:gap-6 md:gap-10">
    {#each metrics as metric (metric.label)}
      <!-- LEARN: per-metric max — each column is normalized within its own scale so a
           metric peaking at 95 doesn't crush a metric peaking at 5.4. Numeric labels
           below the bars carry the absolute values. -->
      ...
      {@const delta =
        metric.before === 0 ? 0 : ((metric.after - metric.before) / metric.before) * 100}
      ...
      <div class="flex flex-col items-center gap-2">
        <div class="flex items-end justify-center gap-3 md:gap-4 h-40 md:h-48 w-full">
          ...
          <div
            class={[
              'w-10 md:w-12 rounded-t',
              isPositive ? 'bg-konpeki dark:bg-konpeki' : 'bg-red-600/70',
            ]}
        ...
        <div class="text-center mt-2">
          ...
          <div
            class={[
              'font-sans text-xs md:text-sm leading-none mt-1',
              isPositive ? 'text-konpeki dark:text-konpeki' : 'text-red-600 dark:text-red-400',
            ]}
          >
            {formatDelta(delta)}
          </div>
